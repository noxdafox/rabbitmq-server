...
process_instruction({discard, ChPid, Flow,
    ...
    {ok, State1 #state { backing_queue_state = BQS1 }};
%% Only during a rolling update while handling discard callbacks
%% where the master hasn't yet adopted the new callback signature.
process_instruction({discard, ChPid, Flow, MsgId}, State) ->
    maybe_flow_ack(ChPid, Flow),
    {ok, publish_or_discard(discarded, ChPid, MsgId, State)};
